...
| medium | 2–4 个候选值 / 口径歧义（含税与否、报价口径、联合体与否） | 列出候选项 + 依据，向用户确认后才写入 |
| low | 无任何来源 | 必须提问（含「自行填写」选项），禁止臆测 |

槽位 = 切片 docx 中的占位：段落空白槽（`____`、`【】`、`（ ）`、冒号空白）与表格占位单元格。槽位 id 稳定格式：段落 `P{段号}#{第几个占位}`、表格 `T{表号}:R{行}C{列}`。
